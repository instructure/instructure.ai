...
function getProductFromLine(values: string[]): ProductNutritionFacts {
	const group: ProductGroup = toProductGroup(values[22]);
	const permissions: ProductNutritionFacts["permissions"] = toPermissionsLevel(
		values[23],
	);
	return {
		data: [
...
const fetchCSV = async (): Promise<{ raw: string; parsed: string[][] }> => {
	let data: string = "";
	try {
...
const fetchProductsFromCSV = async (): Promise<Products> => {
	...
	for (const values of rows) {
		...
		if (!uid) continue;
		products[uid] = getProductFromLine(values);
	}
	return products;
};
...
export { getProductFromCSV, getProductsFromCSV, getProductFromLine, fetchCSV };
