...
module.exports = class CustomCoveragePercentReporter extends ReportBase {
  ...
  onEnd(root) {
    ...
    const totalAvg =
      (totals.statements + totals.branches + totals.functions + totals.lines) /
      4;
    ...
    if (Array.isArray(fileNodes) && fileNodes.length) {
      visitNodes(fileNodes, (node) => {
        ...
        const absPath =
          (fc && (fc.path || (fc.data && fc.data.path))) || undefined;
        const relPath = absPath
          ? relFromCwd(absPath)
          : node.displayShortName || node.name || "unknown";
      ...
      if (node.__metrics) {
        ...
        return {
          ...
          uncovered_line_numbers: node.__metrics.uncovered_line_numbers
            ? node.__metrics.uncovered_line_numbers
                .split(/,|-/)
                .map(Number)
                .filter(Boolean)
            : [],
    ...
    const printTree = (node, indent = 2, isTopLevel = false) => {
      ...
      if (node.__metrics) {
        ...
        if (m.uncovered_line_numbers) {
          w.println(
            `${" ".repeat(indent)}uncovered_line_numbers: "${m.uncovered_line_numbers}"`,
          );
        }
      }
    };
