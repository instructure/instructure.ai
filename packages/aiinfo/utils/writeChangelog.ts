...
function getObjectDiffs(objA: unknown, objB: unknown, path: string[] = []): Array<{ path: string[], oldValue: unknown, newValue: unknown }> {
  ...
  for (const key of keys) {
    ...
    if (typeof valA === "object" && valA !== null && typeof valB === "object" && valB !== null) {
      ...
      diffs.push({ path: [...path, key], oldValue: valA, newValue: valB });
    }
  }
  return diffs;
}
