	...
	try {
		...
		if (!existsSync(entryDir)) {
			try {
				...
				throw new Error(`Failed to create directory '${entryDir}': ${err instanceof Error ? err.message : String(err)}`);
			}
		}
		...
		try {
			...
			throw new Error(`Failed to generate entry content: ${err instanceof Error ? err.message : String(err)}`);
		}
		...
		try {
			...
			throw new Error(`Failed to write entry file '${indexPath}': ${err instanceof Error ? err.message : String(err)}`);
		}
	} catch (err) {
		throw err;
	}
