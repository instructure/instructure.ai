...
const paramsToPendo = (params: string | null): RoadmapFeatures | null => {
  ...
  const stages = Array.from(new Set(p.results.map(result => result.feature.stage))).filter(Boolean) as string[];
  const products = Array.from(new Set(p.results.map(result => result.product.name))).filter(Boolean) as string[];
  const productAreas = Array.from(new Set(p.results.map(result => result.product.area))).filter(Boolean) as string[];
  const labels = Array.from(new Set(p.results.flatMap(result => result.feature.labels))).filter(Boolean) as string[];
